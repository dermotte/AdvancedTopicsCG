...
function setup() {
    ...
    while (visited < size_x * size_y && --steps > 0) nextStep();
    removeDeadEnds();
    strokeWeight(4);
}
            ...
            if (walls > 2 && cell.edges.length > 2) {
                ...
                for (let i = 0; i < cell.edges.length; i++) {
                    if (cell.edges[i].isWall) {
                        let sq = maze.countSquares();
                        cell.edges[i].isWall = false;
                        // Trying to avoid squares ...
                        if (sq < maze.countSquares())
                            cell.edges[i].isWall = true;
                        else
                            break;
                    }
                }
    ...
    countSquares() {
        ...
        for (let r = 0; r < this.rows-1; r++) {
            for (let c = 0; c < this.cols-1; c++) {
                let cell = this.get(c, r);
                let walk = 0;
                if (cell.hasWall("e")) continue;
                if (cell.hasWall("s")) continue;
                cell = this.get(c+1, r+1);
                if (cell.hasWall("n")) continue;
                if (cell.hasWall("w")) continue;
                result++;
            }
        }
        return result;
