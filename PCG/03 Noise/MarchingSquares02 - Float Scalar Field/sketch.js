...
function draw() {
  ...
  for (let i = 0; i < cols; i++) {
    ...
    for (let j = 0; j < rows; j++) {
      ...
      weight[i][j] = noiseVal;
      grid[i][j] = Math.floor(noiseVal + 0.55);
    }
  }
...
function w(x, y) {
  // return resolution * sqrt(x.z / (x.z+y.z));
  return resolution * (x.z / (x.z+y.z));
}
...
function paintRectangle(i, j, caseID) {
  ...
  /*
    ...
    d-cd-c
  */
  let a = createVector(r*i, r*j, weight[i][j]);
  let b = createVector(r*i + r, r*j, weight[i+1][j]);
  let c = createVector(r*i + r, r*j + r, weight[i+1][j+1]);
  let d = createVector(r*i, r*j + r, weight[i][j+1]);
  
  let ab = createVector(a.x + w(a,b), a.y);
  let bc = createVector(b.x, b.y + w(b,c));
  let cd = createVector(d.x + w(d,c), d.y);
  let da = createVector(a.x, a.y + w(a,d));
